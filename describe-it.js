...
const it = (() => {
  ...
  return (description, testFunction) => {
    ...
    if (threw) {
      console.groupCollapsed(`%cFAIL: ${description}`, 'font-weight: bold; color: red;');
    } else {
      if (consoleCalls.length === 0) {
        console.log(`%cPASS: ${description}`, 'font-weight: bold; color: green;');
        itIsCalled = false;
        return;
      }
      console.groupCollapsed(`%cPASS: ${description}`, 'font-weight: bold; color: green;');
    }
